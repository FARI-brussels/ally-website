<template>
  <div class="main">
    <div
      class="search-container color-gray-light-mode-900"
    ...
    <div class="content-wrapper">
      <template v-for="cat in categoryConfigs" :key="cat.slug">
        <transition name="fade">
          <div
            v-if="
              selectedCategory.some((opt) => opt.value === cat.slug) &&
...
const pageData = computed<PageData>(() => {
  ...
  const blocksPage =
    pagesValue && typeof pagesValue === "object" && "blocks" in pagesValue
      ? (pagesValue["blocks"] as {
...
const selectedCategory = ref<OptionProps[]>(
  categories.filter((opt) => opt.value !== null),
);
const selectedCost = ref<OptionProps[]>(
  cost.filter((opt) => opt.value !== null),
);
const selectedDifficulty = ref<OptionProps[]>(
  difficulty.filter((opt) => opt.value !== null),
);
const selectedEffort = ref<OptionProps[]>(
  effort.filter((opt) => opt.value !== null),
);
...
function filterBlocks({
  ...
  return blocks.filter((block) =>
    searchInObject(block as unknown as Record<string, unknown>),
  );
}
...
function getBlocksForCategory(slug: CategorySlug) {
  return computed(() => {
    if (
      ...
      !selectedCost.value.length ||
      !selectedDifficulty.value.length ||
      !selectedEffort.value.length
    )
      return [];
    ...
    return filteredBlocks.value
      .filter(
        (block) =>
          isCategorySlug(block.category?.slug) && block.category?.slug === slug,
      )
      .filter((block) => {
        return selectedDifficulty.value.some(
          (opt: OptionProps) => opt.value === block.effort,
        );
      })
      .filter((block) => {
        return selectedCost.value.some(
          (opt: OptionProps) => opt.value === block.cost,
        );
      })
      .filter((block) => {
        return selectedEffort.value.some(
          (opt: OptionProps) => opt.value === block.effort,
        );
      })
      .map((block) => ({
        ...
        id: block.id,
        title:
          typeof block.title === "object" && block.title
            ? (block.title[locale.value] ?? "")
            : "",
        description:
